...
import { initSingleUserOwner } from '../services/DevAuth';
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    ...
    if (!isDev) {
      return res.status(403).json({
        ...
        error: {
          code: 'FORBIDDEN',
          message: 'Dev endpoints only available in development mode',
        },
      });
    }

    const auth = authConfig();
    if (auth.mode === 'single-user') {
    ...
    if (!user) {
      return res.status(404).json({
        ...
        error: {
          code: 'NOT_FOUND',
          message: 'Local test user not found. Restart server to auto-create.',
        },
      });
    }
