...
const privatePackageAllowlist = new Set(['@codesoul-co/hypha-workcache']);
const failures = [];
...
const manifests = fs
  ...
  .filter(({ manifest }) => manifest.name?.startsWith('@codesoul-co/hypha-'));
...
if (failures.length > 0) {
  ...
  process.stdout.write(
    `Validated ${manifests.filter(({ manifest }) => manifest.private === false).length} publishable @codesoul-co/hypha-* packages at ${rootManifest.version}.\n`
  );
}
