...
`@hypha/workcache` 是从事件流派生的类型化运行时缓存，用于复用 agent 运行过程中的 artifact。它消费现有 Hypha events，将事件映射到 `PlanTree`、`ComputationTree`、`ToolTree`、`ObservationTree`、`VerificationTree`、`MemoryTree` 或 `PromptPrefixTree`，并存储 `CacheBlock`，不改变 DomainPack、Session、Run 或 Event 语义。

通过 `HYPHA_WORKCACHE=memory` 或 `HYPHA_WORKCACHE=sqlite` 开启；默认 `off` 不追加 `workcache.*` 事件。SQLite 文件由 `HYPHA_WORKCACHE_SQLITE_PATH` 设置，`HYPHA_WORKCACHE_PROMPT_BUDGET_TOKENS` 控制 prompt prefix materialization 的预算。

WorkCache 与 Serving Cache 分离。Serving Cache 复用 exact LLM API response；WorkCache 组织 event-derived runtime artifacts。Tool block 只允许 read-only side effect、稳定 args、permission scope 和 validity metadata。Verification block 需要严格的 source、test、environment hash。
