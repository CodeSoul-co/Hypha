...
Keep all `@codesoul-co/hypha-*` packages on one release version. Upgrade them together rather than mixing
framework revisions:

```bash
npm install --save-exact \
  @codesoul-co/hypha-core@1.0.0 \
  @codesoul-co/hypha-domain@1.0.0 \
  @codesoul-co/hypha-kernel@1.0.0 \
  @codesoul-co/hypha-harness@1.0.0
npm run typecheck
npm test
npm run build
```
