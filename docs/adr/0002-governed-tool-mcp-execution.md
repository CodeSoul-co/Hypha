...
The repository already uses workspaces. The owner engineering specification's single-package
constraint is therefore applied as a no-new-package rule: this work extends existing packages and
does not create another workspace package.
...
7. OCR and video-source contracts live inside the existing Tool module. Concrete PaddleOCR,
   Bilibili parsing, course persistence, credentials, and legal-study behavior remain in LexPlan.
