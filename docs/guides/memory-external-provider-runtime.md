...
`HindsightLocalMemoryBankClient` implements the pinned self-hosted Hindsight HTTP 0.8 dialect.
...
External HTTP clients validate their base URL before resolving or attaching credentials. Mem0 OSS
permits cleartext only on loopback for local development; Mem0 Platform and Vertex Memory Bank
require HTTPS. Provider URLs containing user-info, query strings, fragments, or non-HTTP schemes
are rejected at composition time.

Managed asynchronous operations remain provider facts until reconciliation completes. The durable
operation Store records the provider operation name, scope, principal, profile revision, deadline,
attempt count, and operation kind. Update and delete callers receive queued or partial results
until the cloud LRO reaches a successful terminal state; attempt exhaustion is dead-lettered rather
than converted into a synthetic success.

See the [adapter and operations guide](./memory-provider-adapter-and-operations.md) and
[profile configuration](./memory-provider-profiles.md).
