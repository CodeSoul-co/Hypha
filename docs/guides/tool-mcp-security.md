...
- Configure `auditPolicy.includeInput`, `auditPolicy.includeOutput`, and
  `auditPolicy.redactPaths` before production use. Structured schema sensitivity fields are
  metadata for policy and adapter integrations; they do not replace runtime audit redaction.
