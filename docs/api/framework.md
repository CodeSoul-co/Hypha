...
`@hypha/tools` exports `predefinedToolSpecs` and `predefinedToolExamples` for reusable governed tool examples such as `web.search`. `@hypha/mcp` exports `classicMCPIntegrationSpec`, `classicMCPCapabilityDescriptors`, `classicMCPExampleRequests`, and `createClassicMCPMockGateway()` for executable examples. The MCP preset covers `filesystem.read_file`, `fetch.fetch`, `time.now`, and `search.web_search`; each capability normalizes to `ToolSpec` and runs through `GovernedToolRunner` with normal policy, schema validation, and trace events.
