...
| `@hypha/adapters-local` | SQLite/JSON/file/vector local adapters and local runtime engine wiring.                                                     |
...
`FrameworkEvent` fields include `id`, `type`, `runId`, optional `workspaceId`, `sessionId`, `stepId`, `agentId`, `fsmState`, `timestamp`, `payload`, and `metadata`. Durable runtime events may also carry `streamId`, `streamSequence`, `globalSequence`, `idempotencyKey`, `correlationId`, and `causationId` so event streams can be appended idempotently, ordered per stream, correlated across commands and activities, and projected without mutable session state.

`AppendOnlyEventStore` extends the basic `EventStore` with `appendToStream(event, options)`, `getStream(streamId)`, and `getStreamRevision(streamId)`. `appendToStream` assigns per-stream and global sequence numbers, rejects duplicate event ids, supports expected stream revision checks, and returns duplicate results for repeated idempotency keys instead of appending the same logical fact twice.

`RuntimeCommand` is the transport-neutral envelope for session and run operations. `RuntimeCommandProcessor` in `@hypha/harness` validates commands, enqueues them by user/session through `RuntimeCommandQueue`, records `runtime.command.enqueued`, publishes `runtime.commands`, drains commands in FIFO order, maps known commands such as `run.create`, `run.start`, and `run.cancel` to framework events, records `runtime.command.applied`, and publishes `runtime.events`. `InMemoryRuntimeCommandQueue` and `InMemoryMessageBus` are deterministic reference adapters for tests and local development; production adapters must preserve the same idempotency, ordering, correlation, and causation semantics.

`RuntimeDeliveryStore` defines inbox/outbox durability without binding Hypha to a message transport SDK. Delivery records are enqueued idempotently, leased by worker, acknowledged with a lease token, negatively acknowledged for retry, and moved to `dead_lettered` after `maxAttempts`. `InMemoryRuntimeDeliveryStore` is the reference adapter for local tests. `RuntimeLeaseCoordinator` defines resource leases with fencing tokens; `RuntimeCommandProcessor` can use it to acquire a session lease via `runtimeSessionLeaseResource({ userId, sessionId })` before dequeuing commands, so concurrent workers do not process the same session queue.

`defaultRuntimeLoopFSMProcessSpec` models the interactive agent loop as FSM-controlled states rather than an ad hoc nested loop. It separates input acceptance, steering drain, context transform, model streaming, activity prepare/execute/finalize, follow-up drain, and terminal states. `RuntimeLoopContextTransformer` and `RuntimeModelMessageConverter` keep context-window management separate from provider message formatting. Streaming output is represented by append-only `runtime.message.started`, `runtime.message.updated`, and `runtime.message.completed` events; `RuntimeLoopProjector` derives `RuntimeLoopView` and ordered `RuntimeRenderEvent` values for surfaces after applying each source event.

`RuntimeStateAttemptExecutor` binds FSM state attempts to side-effect ports. It acquires a per-attempt lease, creates a fenced `RuntimeStateAttempt`, records state-attempt and activity lifecycle events, calls a supplied `RuntimeActivityPort`, and records `runtime.state_attempt.completed`, `runtime.state_attempt.waiting`, `runtime.state_attempt.failed`, or `runtime.state_attempt.cancelled`. The executor does not implement model, tool, memory, execution, human, or custom activity internals; those remain behind their ports and governance layers.

`RuntimeStateAttemptRecoveryExecutor` recovers projected waiting attempts. `projectWaitingRuntimeStateAttempts(events)` derives open attempts from the append-only event stream. Recovery records `runtime.state_attempt.reconciled`, calls `RuntimeActivityPort.reconcile(activityId)`, records `runtime.activity.reconciled`, and appends the next state-attempt status event. Non-waiting attempts are rejected so completed or failed work is not replayed accidentally.

`RuntimeRecoveryScanner` is the scheduler-facing layer for recovery. It scans the event stream for projected waiting attempts, applies `RuntimeRecoveryScanPolicy` filters such as `includeRunIds`, `excludeStateIds`, `minWaitMs`, and `limit`, resolves the appropriate activity port through `RuntimeActivityPortResolver`, and delegates each selected attempt to `RuntimeStateAttemptRecoveryExecutor`. Each scan can carry a `scanId`, and recovery ids include that scan boundary so repeated recovery passes remain observable instead of collapsing into the same event ids.

`RuntimeRecoveryWorker` wraps scanner execution with a worker-level lease. `runOnce({ scanId, policy })` acquires a shared recovery resource lease, returns `busy` when another worker owns it, and otherwise runs one bounded recovery scan. It does not own timers, deployment topology, or business activity implementations; app surfaces can schedule it from cron, a queue worker, or a service loop while preserving the same FSM and event-first recovery semantics.

`RuntimeLoopRunner` is the minimal package-level loop orchestration path. It advances `defaultRuntimeLoopFSMProcessSpec`, drains steering and follow-up queues, records context transform and conversion events, executes the model state through `RuntimeStateAttemptExecutor`, appends loop and turn completion events, and returns the event-derived loop projection. Concrete activity semantics remain behind `RuntimeActivityPort`.

`ServerRuntimeAdapter` lets app surfaces consume package-level runtime projections without defining runtime behavior in routes. The server runtime API uses it to expose `GET /runtime/runs/:runId/loop` for loop render projection and `GET /runtime/runs/:runId/state-attempts` for state-attempt and waiting-attempt projection.

Common event types include `session.created`, `run.created`, `run.started`, `run.waiting_human`, `run.cancelled`, `runtime.loop.started`, `runtime.loop.completed`, `runtime.turn.started`, `runtime.turn.completed`, `runtime.command.enqueued`, `runtime.command.applied`, `runtime.command.rejected`, `runtime.state_attempt.started`, `runtime.state_attempt.waiting`, `runtime.state_attempt.completed`, `runtime.state_attempt.failed`, `runtime.state_attempt.cancelled`, `runtime.state_attempt.reconciled`, `runtime.steering.drained`, `runtime.follow_up.drained`, `runtime.context.transformed`, `runtime.context.converted`, `runtime.message.started`, `runtime.message.updated`, `runtime.message.completed`, `runtime.activity.prepared`, `runtime.activity.finalized`, `runtime.activity.reconciled`, `fsm.state.entered`, `react.step.completed`, `agent.reasoning.completed`, `inference.completed`, `model.call.completed`, `tool.call.completed`, `memory.write.committed`, `context.compacted`, `human.review.requested`, `human.review.approved`, `human.review.rejected`, `eval.completed`, `replay.completed`, and `regression.completed`.
...
| -------------------------- | ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SQLiteEventStore`         | SQLite or JSON fallback | Append-only event store and trace recorder for replay, audit, regression, and projection. Uses `node:sqlite` when available, otherwise `better-sqlite3`, with JSON sidecar fallback only in `auto` mode. |
...
| `LocalRuntimeCommandQueue` | SQLite or JSON fallback | Durable `RuntimeCommandQueue` implementation with per-session FIFO dequeue and command idempotency.                                                                                          |
| `LocalRuntimeDeliveryStore` | SQLite or JSON fallback | Durable runtime inbox/outbox records implementing `RuntimeDeliveryStore`, including idempotent enqueue, worker leasing, ack/nack retry, and dead-lettering.                                   |
| `LocalRuntimeLeaseCoordinator` | SQLite or JSON fallback | Durable `RuntimeLeaseCoordinator` implementation with fencing tokens for local workers and self-hosted single-node deployments.                                                               |
| `LocalRuntimeMessageBus` | SQLite or JSON fallback | `MessageBus` implementation backed by the durable delivery outbox, with idempotent publish and in-process subscribers for local workers.                                                        |
...
`createLocalRuntimePersistence(options)` returns `structured`, `commandQueue`, `delivery`, `leases`, and `bus` backed by `SQLiteStructuredStore`. Use it to replace the harness in-memory command queue, delivery, message bus, and lease implementations when a local or self-hosted runtime needs restart-safe command/event delivery and fenced recovery. `LocalRuntimeCommandQueue`, `LocalRuntimeDeliveryStore`, `LocalRuntimeMessageBus`, and `LocalRuntimeLeaseCoordinator` depend only on harness contracts, so production Redis, Kafka, Postgres, or cloud adapters can preserve the same behavior without depending on the local adapter package.

`createLocalRuntimeEngine(options)` composes `createLocalStorageBackbone()`, `createLocalRuntimePersistence()`, `RuntimeCommandProcessor`, `RuntimeStateAttemptExecutor`, `RuntimeStateAttemptRecoveryExecutor`, `RuntimeRecoveryScanner`, optional `RuntimeRecoveryWorker`, `RuntimeLoopRunner`, and `ServerRuntimeAdapter`. It is the local/self-hosted wiring path for running the event-first runtime with append-only event streams, durable command queues, durable outbox messages, and fenced FSM state-attempt execution. Pass `recoveryResolver` when the local engine should expose a leased `recoveryWorker` for restart-safe waiting-attempt reconciliation. Business workflows still remain outside Hypha; applications pass their domain context and activity ports into these framework-level components.

`SQLiteEventStore` reads append-only streams in `globalSequence` order when sequence metadata is present, falling back to timestamp/id ordering for legacy events. `SQLiteEventStore.exportJsonl(filePath, filter?)` writes filtered events as newline-delimited JSON and returns the exported count. `SQLiteEventStore.importJsonl(filePath)` appends those events into the configured event store and returns the imported count. Use these APIs for replay fixtures, audit snapshots, and regression traces.
