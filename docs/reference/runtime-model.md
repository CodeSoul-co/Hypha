...
`@codesoul-co/hypha-core` exposes provider-neutral runtime contracts together with in-memory reference
...
`BoundedFSMDriver` in `@codesoul-co/hypha-harness` advances an FSM only while a transition is supported by
...
`@codesoul-co/hypha-harness` exposes `MessageBus` and `InMemoryMessageBus` as the transport
