...
| `docker`         | Process, filesystem, namespace, resource, mount, security, network, and image-digest controls are mandatory | Stop, forced kill, receipt capture, and container cleanup are idempotent and reconciled                                     | `DockerExecutionProvider` provides a single-use, digest-pinned Docker adapter; unsupported policies fail closed |
