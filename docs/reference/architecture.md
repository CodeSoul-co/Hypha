...
| Internal WorkCache workspace package | Event-derived typed runtime cache blocks, WorkGraph scheduling view, hot-indexed typed cache forest, memory/SQLite stores, and audit events.                     | Public npm surface in v1.0.0, source-of-truth events, provider response cache, MessageTree, or KVPrefixTree. |
...
`fsm` remains a separate package because the Harness requires one reusable, framework-owned capability state machine. It tracks lifecycle, context construction, reasoning, action selection, policy checks, governed activity execution, observation, verification, memory synchronization, recovery, human review, and terminal outcomes. Recovery states such as `Recovering`, `Compensating`, and `Quarantined` are visible scheduling and evidence boundaries rather than a hidden exception loop.
...
apps/server -> internal WorkCache package -> @codesoul-co/hypha-core
...
The internal WorkCache workspace package consumes existing runtime events and materializes typed
