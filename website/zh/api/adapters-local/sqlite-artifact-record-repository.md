...
| Symbol | 种类 | 签名 | 说明 |
| --- | --- | --- | --- |
| `SQLiteArtifactRecordRepository` | 类 | <code>new SQLiteArtifactRecordRepository(options: SQLiteArtifactRecordRepositoryOptions): SQLiteArtifactRecordRepository</code> | SQ Lite Artifact Record Repository 的运行时实现；公开构造函数与成员见下表。 |
| `SQLiteArtifactRecordRepositoryOptions` | 接口 | <code>interface SQLiteArtifactRecordRepositoryOptions</code> | SQ Lite Artifact Record Repository Options 的字段契约；完整字段见下表。 |

## `SQLiteArtifactRecordRepository` 公开成员
...
## `SQLiteArtifactRecordRepositoryOptions` 契约字段

| 成员 | 种类 | 签名 | 说明 |
| --- | --- | --- | --- |
| `busyTimeoutMs` | 属性 | <code>busyTimeoutMs: number</code> | busy Timeout Ms 字段。 |
| `filename` | 属性 | <code>filename: string</code> | filename 字段。 |
| `id` | 属性 | <code>id: string</code> | id 字段。 |
