...
| Symbol | 种类 | 签名 | 说明 |
| --- | --- | --- | --- |
| `SQLiteExecutionStoreError` | 类 | <code>new SQLiteExecutionStoreError(code: SQLiteExecutionStoreFoundationErrorCode, message: string, details?: Record&lt;string, unknown&gt; &#124; undefined, cause?: unknown): SQLiteExecutionStoreFoundationError</code> | SQ Lite Execution Store Error 的运行时实现；公开构造函数与成员见下表。 |

## `SQLiteExecutionStoreError` 公开成员

| 成员 | 种类 | 签名 | 说明 |
| --- | --- | --- | --- |
| `cause` | 属性 | <code>cause: unknown</code> | cause 字段。 |
| `code` | 属性 | <code>code: SQLiteExecutionStoreFoundationErrorCode</code> | code 字段。 |
