...
describe('CachedMemoryManagementProvider', () => {
  ...
  it('keys every required user, tenant, workspace, session, run, agent and domain scope', async () => {
    const keys: string[] = [];
    const search = vi.fn(async (request: ManagedMemorySearchRequest) => resultsFor(request));
    ...
    const provider = new CachedMemoryManagementProvider({
      ...
      requiredScopeFields,
      trace: (event) => {
        if (event.type === 'memory.cache.lookup' && event.key) keys.push(event.key);
      },
    });
    const request = searchRequest('operation:full-scope');
    const fullScope = {
      userId: 'user_01',
      tenantId: 'tenant_01',
      ...
      runId: 'run_01',
      agentId: 'agent_01',
      domainPackId: 'domain_01',
    };

    await provider.search({ ...request, scope: fullScope });
    for (const field of requiredScopeFields) {
      await provider.search({
        ...request,
        operationId: 'operation:changed-' + field,
        scope: { ...fullScope, [field]: fullScope[field] + ':changed' },
      });
    }

    expect(search).toHaveBeenCalledTimes(requiredScopeFields.length + 1);
    expect(new Set(keys).size).toBe(requiredScopeFields.length + 1);
  });

  it('bypasses Store, singleflight and stale reuse when a required scope is incomplete', async () => {
    const cache = {
      getScopeRevision: vi.fn(async () => '0'),
      get: vi.fn(async () => null),
      set: vi.fn(async () => undefined),
      delete: vi.fn(async () => undefined),
      invalidateScope: vi.fn(async () => 0),
    };
    const search = vi.fn(async (request: ManagedMemorySearchRequest) => resultsFor(request));
    const events: string[] = [];
    const provider = new CachedMemoryManagementProvider({
      ...
      cache,
      providerRevision: 'provider:v1',
      requiredScopeFields: ['userId', 'workspaceId', 'sessionId', 'runId', 'agentId'],
      trace: (event) => {
        if (event.type === 'memory.cache.bypass' && event.reason) events.push(event.reason);
      },
    });
    const request = {
      ...searchRequest('operation:incomplete-one'),
      scope: {
        userId: 'user_01',
        workspaceId: 'workspace_01',
        sessionId: 'session_01',
    ...
    await Promise.all([
      ...
      provider.search({ ...request, operationId: 'operation:incomplete-two' }),
    ]);

    expect(search).toHaveBeenCalledTimes(2);
    expect(cache.getScopeRevision).not.toHaveBeenCalled();
    expect(cache.get).not.toHaveBeenCalled();
    expect(cache.set).not.toHaveBeenCalled();
    expect(events).toEqual(['scope_incomplete', 'scope_incomplete']);
  });

  it('reauthorizes every hit and never authorizes a Memory write', async () => {
    const search = vi.fn(async (request: ManagedMemorySearchRequest) => resultsFor(request));
    const add = vi.fn(async () => {
      throw new Error('write policy denied');
    });
    let decision = {
      allowed: true,
    ...
    const provider = new CachedMemoryManagementProvider({
      provider: { ...testProvider(search), add },
      cache: new InMemoryMemorySearchCacheStore(),
      providerRevision: 'provider:v1',
      cacheAuthorization: { authorize },
      requireCacheAuthorization: true,
      now: () => 1_000,
    });
    const request = searchRequest('operation:authorized');

    await provider.search(request);
    await provider.search({ ...request, operationId: 'operation:authorized-hit' });
    expect(search).toHaveBeenCalledOnce();
    expect(authorize).toHaveBeenCalledTimes(2);

    decision = { ...decision, allowed: false, reason: 'policy revoked' };
    await expect(
      provider.search({ ...request, operationId: 'operation:revoked' })
    ).rejects.toMatchObject({ code: 'MEMORY_POLICY_REJECTED' });
    expect(search).toHaveBeenCalledOnce();
    ...
    await provider.search({ ...request, operationId: 'operation:policy-v2' });
    expect(search).toHaveBeenCalledTimes(2);
    ...
    await expect(
      provider.search({ ...request, operationId: 'operation:expired' })
    ).rejects.toMatchObject({ code: 'MEMORY_POLICY_REJECTED' });

    await expect(
      provider.add({
        operationId: 'operation:write-denied',
        principal: request.principal,
        scope: request.scope,
        profileRef: request.profileRef,
        input: 'must still pass write policy',
        memoryType: 'semantic',
        source: { type: 'user_message' },
      })
    ).rejects.toThrow('write policy denied');
    expect(add).toHaveBeenCalledOnce();
  });

  it('quarantines cached results carrying forged provider evidence', async () => {
    let record: MemorySearchCacheRecord | null = null;
    const cache: MemorySearchCacheStore = {
      getScopeRevision: async () => '0',
      get: async () =>
        record
          ? ({
              ...record,
              providerEvidence: { terminal: 'committed' },
            } as unknown as MemorySearchCacheRecord)
          : null,
    ...
    const request = searchRequest('operation:evidence-one');

    await provider.search(request);
    await provider.search({ ...request, operationId: 'operation:evidence-two' });
