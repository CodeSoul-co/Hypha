...
export class NativeMemoryManagementProvider implements MemoryManagementProvider {
  ...
  constructor(private readonly options: NativeMemoryProviderOptions) {
    ...
    this.recordStore = options.recordStore ?? new InMemoryManagedMemoryRecordStore();
    this.idempotencyStore = options.idempotencyStore ?? new InMemoryMemoryIdempotencyStore();
    this.outboxStore = options.outboxStore ?? new InMemoryMemoryIndexOutboxStore();
  ...
  async add(request: MemoryAddRequest): Promise<ManagedMemoryWriteResult> {
    ...
    try {
      ...
      await this.recordStore.transaction(async (store) => {
        if (record.revision === 1) await store.create(record);
        else await store.createVersion(record, record.revision - 1);
  ...
  async update(request: ManagedMemoryUpdateRequest): Promise<ManagedMemoryWriteResult> {
    ...
    await this.recordStore.transaction(async (store) => {
      await store.createVersion(updated, request.expectedRevision!);
      await this.outboxStore.enqueue({
        id: indexJobId,
        operationId: request.operationId,
        memoryId: updated.id,
    ...
    for (const id of targets) {
      ...
      await this.recordStore.transaction(async (store) => {
        ...
        if (request.mode === 'hard') {
          await store.delete(id, request.scope);
        } else {
          const tombstone = await store.updateStatus(
            id,
            request.scope,
            current.revision,
            'deleted',
