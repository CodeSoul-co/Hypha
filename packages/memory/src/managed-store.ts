...
export class InMemoryMemoryIndexOutboxStore implements MemoryIndexOutboxStore {
  ...
  async transaction<T>(fn: (store: MemoryIndexOutboxStore) => Promise<T>): Promise<T> {
    const snapshot = new Map(
      Array.from(this.records.entries()).map(([id, record]) => [id, structuredClone(record)])
    );
    try {
      return await fn(this);
    } catch (error) {
...
export class InMemoryMemoryPersistenceUnitOfWork implements MemoryPersistenceUnitOfWork {
  ...
  constructor(
    readonly recordStore: ManagedMemoryRecordStore = new InMemoryManagedMemoryRecordStore(),
    readonly outboxStore: MemoryIndexOutboxStore = new InMemoryMemoryIndexOutboxStore()
  ) {}

  transaction<T>(fn: (stores: MemoryPersistenceTransaction) => Promise<T>): Promise<T> {
    return this.recordStore.transaction((recordStore) =>
      this.outboxStore.transaction((outboxStore) => fn({ recordStore, outboxStore }))
    );
  }
}
