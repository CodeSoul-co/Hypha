...
export class StructuredMemoryLifecycleTaskStore implements MemoryLifecycleTaskStore {
  ...
  async lease(
    ...
    if (this.options.store.compareAndSet) {
      const candidates = await this.options.store.query<MemoryLifecycleTask>(this.table, {
        where: { type },
      });
      ...
      for (const candidate of selected) {
        const next = leasedLifecycleTask(candidate, ownerId, now, leaseUntil);
        const claimed = await this.options.store.compareAndSet<MemoryLifecycleTask>(
          this.table,
          candidate.id,
          lifecycleRecordVersion(candidate),
          next
        );
    ...
    return this.options.store.transaction(async (transaction) => {
      ...
      for (const candidate of selected) {
        ...
        const next = leasedLifecycleTask(current, ownerId, now, leaseUntil);
  ...
  async renew(
    ...
    if (this.options.store.compareAndSet) {
      const task = await this.options.store.get<MemoryLifecycleTask>(this.table, taskId);
      if (!hasLifecycleLease(task, ownerId, leaseToken, now) || leaseUntil <= now) return false;
      return this.options.store.compareAndSet<MemoryLifecycleTask>(
        this.table,
        taskId,
        lifecycleLeaseVersion(task),
...
function leasedLifecycleTask(
  ...
  return {
    ...
    leaseOwner: ownerId,
    leaseToken: lifecycleLeaseToken(ownerId, task.id, fencingToken),
    leaseExpiresAt: leaseUntil,
    fencingToken,
    updatedAt: now,
  };
...
function lifecycleRecordVersion(task: MemoryLifecycleTask): Partial<MemoryLifecycleTask> {
  return {
    state: task.state,
    attempts: task.attempts,
    updatedAt: task.updatedAt,
    ...(task.fencingToken === undefined ? {} : { fencingToken: task.fencingToken }),
  };
}

function lifecycleLeaseVersion(task: MemoryLifecycleTask): Partial<MemoryLifecycleTask> {
  return {
    ...lifecycleRecordVersion(task),
