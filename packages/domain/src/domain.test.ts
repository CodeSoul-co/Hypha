...
`,
  ...
  it('compiles managed Memory bindings and an immutable dependency snapshot', () => {
    const managedPack = validateDomainPackSpec({
      ...domainPackSpecDefinition.example,
      memoryProfiles: [
        {
          ...memoryProfileSpecExample,
          scopePolicy: {
            ...
            requiredDimensions: ['tenantId', 'userId'],
          },
        },
      ],
          ...
          states: domainPackSpecDefinition.example.workflows[0].states.map((state) =>
            state.id === 'Reasoning'
              ? {
                  ...
                  autoCapture: true,
                  readPolicyRef: { id: 'policy.default', version: '0.0.0' },
                  writePolicyRef: { id: 'policy.default', version: '0.0.0' },
                  allowedMemoryTypes: ['episodic', 'semantic'],
                }
              : state
          ),
        },
    ...
    expect(compiled.bindings.extractionProfile?.id).toBe(memoryExtractionProfileSpecExample.id);
    expect(
      compiled.bindings.workflowStates.find((state) => state.stateId === 'Reasoning')?.memory
    ).toMatchObject({
      memoryProfileRef: { id: memoryProfileSpecExample.id, version: '1.0.0' },
    ...
    expect(compiled.memoryDependencySnapshot).toMatchObject({
      ...
      contextProfileRef: { id: contextProfileSpecExample.id, version: '1.0.0' },
      extractionProfileRef: { id: memoryExtractionProfileSpecExample.id, version: '1.0.0' },
      scopeTemplate: { tenantId: 'tenant:default' },
      capabilitySnapshot: memoryManagementProviderSpecExample.capabilities,
      createdAt: '2026-07-18T00:00:00.000Z',
    });
    expect(compiled.memoryDependencySnapshot?.stateBindings).toEqual(
      expect.arrayContaining([expect.objectContaining({ stateId: 'Reasoning' })])
    );
  ...
  it('rejects managed Memory bindings with unresolved versions, scopes, or capabilities', () => {
    const base = {
      ...domainPackSpecDefinition.example,
      memoryProfiles: [memoryProfileSpecExample],
      contextProfiles: [contextProfileSpecExample],
      workflows: [
        {
          ...domainPackSpecDefinition.example.workflows[0],
          states: domainPackSpecDefinition.example.workflows[0].states.map((state) =>
            state.id === 'Reasoning'
              ? {
                  ...state,
                  memoryProfileRef: { id: memoryProfileSpecExample.id, version: '9.0.0' },
                  memoryAccessMode: 'read_write' as const,
                }
              : state
          ),
        },
    ...
    expect(() => validateDomainPackSpec(base)).toThrow(/Memory profile version mismatch/);

    const valid = validateDomainPackSpec({
      ...base,
      workflows: [
        {
          ...base.workflows[0],
          states: base.workflows[0].states.map((state) =>
            state.id === 'Reasoning'
              ? { ...state, memoryProfileRef: undefined, memoryAccessMode: 'read_write' as const }
              : state
          ),
        },
      ],
    ...
    ).toThrow(/Memory scope cannot be initialized.*tenantId/);
